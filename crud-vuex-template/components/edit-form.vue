<template>
    <Form ref="{{resourceName}}-form" :model="form" :label-width="100" :rules="rules">
...
<script>
import { update{{upperFirst resourceName}}, create{{upperFirst resourceName}}, get{{upperFirst resourceName}} } from '@/api/{{resourceName}}';
export default {
    data() {
        return {
            form: {
    ...
    methods: {
        async initData(id) {
            ...
            if (this.id) {
                const data = await get{{upperFirst resourceName}}(id);
                Object.assign(this.form, data);
            }
        },
        async submit() {
            const valid = await this.$refs['{{resourceName}}-form'].validate();
            let success;
            if (valid) {
                try {
                    if (this.isEdit) {
                        const res = await update{{upperFirst resourceName}}(this.form);
                        success = res.data.success;
                    } else {
                        const res = await create{{upperFirst resourceName}}(this.form);
                        success = res.data.success;
                    }
                } catch (e) {
        ...
        clearForm() {
            this.$refs['{{resourceName}}-form'].resetFields();
        }
    }
};
</script>
